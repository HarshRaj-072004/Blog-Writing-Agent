    ...
    for kind, payload in try_stream(app, inputs):
        if kind in ("updates", "values"):
            ...
            summary = {
                ...
                "evaluation_ready": current_state.get("evaluation") is not None,
            }
...
if out:
    ...
    with tab_evaluation:
        ...
        if not report:
            ...
            c1.metric(
                "Coverage",
                report.get("coverage_score", 0)
            )

            c2.metric(
                "Citation",
                report.get("citation_score", 0)
            )

            c3.metric(
                "Structure",
                report.get("structure_score", 0)
            )

            c4.metric(
                "Hallucination Risk",
                report.get("hallucination_risk", 0)
            )

            st.metric(
                "Overall Score",
                f"{report.get('overall_score', 0):.2f}"
            )

            
            review_status = out.get("human_review_status")

            if review_status == "AUTO_APPROVED":
                st.success("Auto Approved")

            elif review_status == "REVIEW_NEEDED":
                st.warning(" Human Review Required")

            elif review_status == "SKIPPED":
                st.info("Human Review Skipped")
